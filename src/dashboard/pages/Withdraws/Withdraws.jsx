...
export default function WithDraw() {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) =>
      ...
      (user?.user.type || "").toLowerCase().includes(searchTerm.toLowerCase())||
      String(user?.user.phone || "").toLowerCase().includes(searchTerm.toLowerCase())||
      String(user?.withdrawal_amount || "").toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, users]);
          ...
          <TableContainer>
            <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
              <TableHead className={`table-head ${isSmallScreen && "hidden"}`}>
                <TableRow>
                  <TableCell align="left" sx={{ color: "#AAADAF" }}>User Name</TableCell>
                  <TableCell align="center" sx={{ color: "#AAADAF" }}>Withdraw-ID</TableCell>
